...
Open the repository as an RStudio project, which will activate `renv`. Then run:
...
The following data files (EHR exports) need to be placed in the `data-raw` directory:
...
1. import-data.Rmd
2. estimator-superlearner.Rmd - takes 3+ days to run, depending on CPU cores.
3. variable-importance.Rmd
4. interpretation.Rmd
5. decision-analysis.Rmd

Note: in import-data.Rmd the GLRM grid search is disabled by default due to its CPU-intensive nature. To re-enable it, go to the section `glrm_grid_search` and set eval = TRUE the RMarkdown header (current value is FALSE).

## ck37r package

Examples and demonstration results for the accompanying `ck37r` package are provided in its [github repository](https://github.com/ck37/ck37r).

## License

The contents of this repository are distributed under the MIT license.

&copy; Chris J. Kennedy, 2021