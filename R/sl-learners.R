...
SL.ranger_fast = function(...) SuperLearner::SL.ranger(..., num.trees = 700, #min.node.size = 20L,
                                                       num.threads = get_cores())

SL.ranger_2000 = function(...) SuperLearner::SL.ranger(..., num.trees = 2000, #min.node.size = 20L,
                                                       num.threads = get_cores())
...
get_cores = function(default = RhpcBLASctl::get_num_cores()) {
  getOption("sl.cores",
            ifelse(exists("conf") && "max_cores" %in% names(conf),
                   conf$max_cores, default))
}
